...
const TooltipContent = ({ killer }: { killer: Person }) => (
  <div className="space-y-1 text-left">
    {killer.eyeColor && (
      <div>
        <span className="font-semibold">Eye Color:</span> {killer.eyeColor}
      </div>
    )}
    {killer.hairColor && (
      <div>
        <span className="font-semibold">Hair Color:</span> {killer.hairColor}
      </div>
    )}
    {killer.location && (
      <div>
        <span className="font-semibold">Location:</span>{' '}
        {killer.location.name || 'Unknown'}
    ...
    {killer.height && (
      <div>
        <span className="font-semibold">Height:</span> {killer.height}
      </div>
    )}
    {killer.weight && (
      <div>
        <span className="font-semibold">Weight:</span> {killer.weight}
      </div>
    )}
    {killer.passportID && (
      <div>
        <span className="font-semibold">Passport ID:</span> {killer.passportID}
      </div>
    )}
  </div>
);
...
export const TablePage = () => {
  ...
  const [reassignAndDelete, { isLoading: isDeleting }] =
    useReassignAndDeleteDragonMutation();
  ...
  const {
    ...
    error,
    isLoading,
    refetch: refetchDragons,
  } = useGetDragonsQuery(getDragonApiArg);
  ...
  const updateFilter = <K extends keyof DragonFilter>(
    key: K,
    value: DragonFilter[K] | undefined,
  ) => {
  ...
  const handleConfirmDelete = async (newOwnerId: number) => {
    ...
    try {
      ...
      const errorMessage =
        apiError.data?.message || 'Failed to process request.';
              ...
              <td className="border border-gray-300 px-2 py-2">
                <div className="flex w-full items-center gap-1">
                  <input
                    ...
                    placeholder="From"
                    value={
                      filterDragon.creationDateRange?.from?.split('T')[0] || ''
                    }
                    onChange={(e) => {
                  ...
                  <input
                    ...
                    placeholder="To"
                    value={
                      filterDragon.creationDateRange?.to?.split('T')[0] || ''
                    }
                    onChange={(e) => {
                      const value = e.target.value
              ...
              <td className="border border-gray-300 px-2 py-2">
                <input
                  ...
                  onChange={(e) =>
                    updateFilter('cave', {
                      ...filterDragon.cave,
                      numberOfTreasures: e.target.value
              ...
              <td className="border border-gray-300 px-2 py-2">
                <select
                  ...
                  onChange={(e) =>
                    updateFilter(
                      'color',
                      (e.target.value as Color) || undefined,
                    )
                  }
                  className="w-full px-2 py-1 border border-gray-300 rounded text-sm"
                >
              ...
              <td className="border border-gray-300 px-2 py-2">
                <select
                  ...
                  onChange={(e) =>
                    updateFilter(
                      'type',
                      (e.target.value as DragonType) || undefined,
                    )
                  }
                  className="w-full px-2 py-1 border border-gray-300 rounded text-sm"
                >
              ...
              <td className="border border-gray-300 px-2 py-2">
                <select
                  ...
                  onChange={(e) =>
                    updateFilter(
                      'character',
                      (e.target.value as DragonCharacter) || undefined,
                    )
                  }
                  className="w-full px-2 py-1 border border-gray-300 rounded text-sm"
                >
