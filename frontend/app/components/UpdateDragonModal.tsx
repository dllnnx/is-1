...
export const UpdateDragonModal = ({isOpen, onClose, dragonToUpdate}: Props) => {
    ...
    const [useExistingPersonLocation, setUseExistingPersonLocation] = useState(true);

    const [newCoordinates, setNewCoordinates] = useState<{x: number; y: number}>({x: 0, y: 0});
    const [newCave, setNewCave] = useState<{depth: number; numberOfTreasures: number}>({depth: 0, numberOfTreasures: 0});
    ...
    const [newHead, setNewHead] = useState<{toothCount: number}>({toothCount: 0});
    const [newPersonLocation, setNewPersonLocation] = useState<{name: string; x: number; y: number}>({name: "", x: 0, y: 0});
    ...
    useEffect(() => {
        if (dragonToUpdate && isOpen) {
            ...
            setNewCoordinates({x: dragonToUpdate.coordinates.x, y: dragonToUpdate.coordinates.y});
            setNewCave({depth: dragonToUpdate.cave.depth, numberOfTreasures: dragonToUpdate.cave.numberOfTreasures});
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        const getCoordinates = () => {
            if (useExistingCoordinates && typeof formData.coordinates === 'object' && formData.coordinates?.id) {
                return formData.coordinates;
            }
            const coords = useExistingCoordinates && typeof formData.coordinates === 'object' 
        ...
        const getCave = () => {
            if (useExistingCave && typeof formData.cave === 'object' && formData.cave?.id) {
                return formData.cave;
            }
            const cave = useExistingCave && typeof formData.cave === 'object'
                ? formData.cave
        ...
        const getKiller = () => {
            if (useExistingKiller && formData.killer && typeof formData.killer === 'object' && formData.killer.id) {
                return formData.killer;
            }
            if (useExistingKiller) {
                return null;
            }
            const person = typeof formData.killer === 'object' && formData.killer
                ? formData.killer
                : {
                    name: newPerson.name,
            ...
            const location = useExistingPersonLocation && newPerson.location?.id
                ? newPerson.location
                : useExistingPersonLocation ? null
                : {
                    id: undefined,
        ...
        const getHead = () => {
            if (useExistingHead && formData.head && typeof formData.head === 'object' && formData.head.id) {
                return formData.head;
            }
            if (useExistingHead) {
                return undefined;
            }
            const head = typeof formData.head === 'object' && formData.head
                ? formData.head
        ...
        try {
            ...
            const errorMessage = e?.data?.message || e?.message || "Failed to update dragon";
                        ...
                        <NestedEntitySelector
                            ...
                            existingEntitySelect={
                                <select
                                    ...
                                    value={typeof formData.coordinates === 'object' && formData.coordinates?.id ? formData.coordinates.id : ""}
                                    onChange={(e) => {
                                        const coord = coordinates.find(c => c.id === parseInt(e.target.value));
                        ...
                        <NestedEntitySelector
                            ...
                            existingEntitySelect={
                                <select
                                    required
                                    className="w-full border rounded px-3 py-2"
                                    value={typeof formData.cave === 'object' && formData.cave?.id ? formData.cave.id : ""}
                                    onChange={(e) => {
                                        const cave = caves.find(c => c.id === parseInt(e.target.value));
                    ...
                    <FormSection title="Related Entities (Optional)" columns={2}>
                        <NestedEntitySelector
                            ...
                            existingEntitySelect={
                                <select
                                    className="w-full border rounded px-3 py-2"
                                    value={typeof formData.killer === 'object' && formData.killer?.id ? formData.killer.id : ""}
                                    onChange={(e) => {
                                        if (e.target.value) {
                            ...
                            newEntityForm={
                                <div className="grid grid-cols-2 gap-3">
                                    ...
                                    <FormSelect
                                        ...
                                        onChange={(val) => setNewPerson({...newPerson, hairColor: val ? val as Color : null})}
                                        options={[
                                    ...
                                    <div className="col-span-2">
                                        <NestedEntitySelector
                                            ...
                                            onToggle={() => {
                                                setUseExistingPersonLocation(!useExistingPersonLocation);
                                            }}
                                            existingEntitySelect={
                                                <select
                                                    className="w-full border rounded px-3 py-2"
                                                    value={newPerson.location?.id ? newPerson.location.id : ""}
                                                    onChange={(e) => {
                                                        if (e.target.value) {
                                            ...
                                            newEntityForm={
                                                <div className="space-y-3 p-3 border rounded">
                                                    <FormInputString
                                                        ...
                                                        onChange={(val) => setNewPersonLocation({...newPersonLocation, name: val})}
                                                    ...
                                                    <div className="grid grid-cols-2 gap-2">
                                                        <FormInputFloat
                                                            ...
                                                            onChange={(val) => setNewPersonLocation({...newPersonLocation, x: val})}
                                                        ...
                                                        <FormInputFloat
                                                            ...
                                                            onChange={(val) => setNewPersonLocation({...newPersonLocation, y: val})}
                        ...
                        <NestedEntitySelector
                            ...
                            existingEntitySelect={
                                <select
                                    className="w-full border rounded px-3 py-2"
                                    value={typeof formData.head === 'object' && formData.head?.id ? formData.head.id : ""}
                                    onChange={(e) => {
                                        if (e.target.value) {
                                            const head = heads.find(h => h.id === parseInt(e.target.value));
