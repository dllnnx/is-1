...
public class JacksonConfiguration {
  ...
  public ObjectMapper objectMapper(Jackson2ObjectMapperBuilder builder) {
    ...
    module.addDeserializer(
        DragonCreateCoordinates.class, new DragonCreateCoordinatesDeserializer());
  ...
  static class DragonCreateCaveDeserializer extends JsonDeserializer<DragonCreateCave> {
    @Override
    public DragonCreateCave deserialize(JsonParser p, DeserializationContext ctxt)
        throws IOException {
  ...
  static class DragonCreateHeadDeserializer extends JsonDeserializer<DragonCreateHead> {
    @Override
    public DragonCreateHead deserialize(JsonParser p, DeserializationContext ctxt)
        throws IOException {
      JsonNode node = p.getCodec().readTree(p);
  ...
  static class DragonCreateCoordinatesDeserializer
      extends JsonDeserializer<DragonCreateCoordinates> {
    @Override
    public DragonCreateCoordinates deserialize(JsonParser p, DeserializationContext ctxt)
        throws IOException {
      JsonNode node = p.getCodec().readTree(p);
  ...
  static class DragonCreateKillerDeserializer extends JsonDeserializer<DragonCreateKiller> {
    @Override
    public DragonCreateKiller deserialize(JsonParser p, DeserializationContext ctxt)
        throws IOException {
      JsonNode node = p.getCodec().readTree(p);
